    ...
    try:
        ...
        if msg_type == 'create_room':
            ...
            if player_id not in room['votes']:
                ...
                await broadcast_all(conn['room_code'], {'type': 'vote_cast', 'data': {'scenarioPlayerId': scenario_player_id, 'scenarios': room['submitted_scenarios']}})
...
async def handle_disconnect(ws):
    ...
    await broadcast_room(room_code, {'type': 'player_left', 'data': {'playerId': player_id, 'playerName': player_name, 'players': room['players']}})
    if not room['players']:
        ...
        await broadcast_room(room_code, {'type': 'host_changed', 'data': {'newHostId': new_host['id'], 'players': room['players']}})
...
async def http_handler(request):
    return web.Response(text="SAGA RPG Server is running!", content_type="text/plain")

async def main():
    port = int(os.environ.get("PORT", 8765))
    log(f"Starting SAGA server on port {port}")
    ...
    await site.start()
    log(f"HTTP health check running on port {port}")

    # WebSocket on different port
    ws_port = port + 1
    async with websockets.serve(ws_handler, '0.0.0.0', ws_port):
        log(f"WebSocket server running on port {ws_port}")
